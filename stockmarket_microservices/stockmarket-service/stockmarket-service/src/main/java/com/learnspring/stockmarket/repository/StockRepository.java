...
public interface StockRepository extends JpaRepository<StockPrice, Integer> {
	...
	@Query(value = "SELECT stock_price_id,company,stock_exchange,date,stock_price " + "FROM stock_price "
			+ "WHERE company=? AND (date BETWEEN (CURDATE()-INTERVAL 1 MONTH) AND CURDATE()) ORDER BY date", nativeQuery = true)
	List<StockPrice> getStockPricesByCompanyLastMonth(long id);

	@Query(value = "SELECT stock_price_id,company,stock_exchange,date,stock_price " + "FROM stock_price "
			+ "WHERE company=? AND (date BETWEEN (CURDATE()-INTERVAL 3 MONTH) AND CURDATE()) ORDER BY date", nativeQuery = true)
	List<StockPrice> getStockPricesByCompanyLastQuarter(long id);
